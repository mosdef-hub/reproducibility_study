    ...
    for (
        ...
        if "NVT" in ensemble and engine == "mcccs":
            ...
            for job in group:
                ...
                pressure_list.append(avg_one_seed_pressure_box1(prod_run_files))
...
def avg_one_seed_pressure_box1(prod_run_files):
    ...
    if len(prod_run_files) == 0:
        return None
    os.system(
        "grep pressure run.prod* | grep kPa | awk '{print $5}' > temp_pressure.txt"
    )
    # os.system("grep 'specific density                        ' run.prod* | awk '{print $6}' ")
    if os.path.exists("temp_pressure.txt"):
        try:
            pressure_list_one_seed = np.genfromtxt("temp_pressure.txt")
        except Exception as e:
            raise e
        finally:
            # now delete the temp file
            os.remove("temp_pressure.txt")
        return np.mean(pressure_list_one_seed)
    else:
        return None
