...
def traj_exists(job):
    ...
    if "NVT" in job.sp.ensemble:
        return job.isfile("trajectory-nvt.gsd")
    elif job.sp.ensemble == "GEMC-NVT":
...
def sanitize_nvt_log(step, job):
    """Sanitize the output logs for NVT simulations."""
    import numpy as np
...
@ex
@Project.operation
@Project.pre(lambda j: j.sp.engine == "mcccs" and "NVT" in job.sp.ensemble)
@Project.pre(has_fort_files)
@Project.post(files_ready)
def replace_keyword_fort_files_nvt(job):
...
def convert_to_txt(job):
    ...
    with job:
        if job.sp.ensemble == "GEMC-NVT":
            ...
            prod_log = sanitize_nvt_log("prod", job)
            os.rename("prod_log.txt", "log-nvt.txt")
        ...
        if job.sp.ensemble == "GEMC-NVT":
            ...
            for filename in traj_files:
                ...
                traj = md.Trajectory(
                    traj.xyz,
                    traj.top,
                    unitcell_lengths=fort12[9::100, 0:3] / 10,
                    unitcell_angles=np.tile(
                        [90.0, 90.0, 90.0], (traj.n_frames, 1)
                    ),
                )
                ...
                prod_number += 1
            comb_traj = md.join(traj_list)
            comb_traj.save_gsd("trajectory-nvt.gsd")
