...
def lammps_equilibrated_npt(job):
    ...
    return job.isfile("equilibrated-npt.restart")
    p = pathlib.Path(".")
...
def lammps_em_nvt(job):
    ...
    modify_submit_scripts(
        in_script_name, job.id, cores=1, ensemble=job.sp.ensemble, stage="em"
    )
...
def lammps_reformat_data(job):
    ...
    print("reformatting data for {}".format(job))
...
def lammps_create_gsd(job):
    ...
    print("creating trajectory-npt.gsd for {}".format(job))
...
def modify_submit_scripts(
    filename, jobid, cores=8, ensemble="NPT", stage="stage"
):
    """Modify the submission scripts to include the job and simulation type in the header."""
    with open("submit.sh", "r") as f:
        lines = f.readlines()
        lines[2] = "#SBATCH -J lammps_{}_{}_{}\n".format(
            ensemble, stage, jobid[0:6]
        )
