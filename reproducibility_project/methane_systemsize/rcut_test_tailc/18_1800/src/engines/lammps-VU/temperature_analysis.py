...
def main():
    ...
    data_path = "temperature_analysis_data"
    ...
    for (
        ...
        base_dir = os.getcwd()
        if "NPT" in ensemble and engine == "lammps-VU":
            temp_list = []
            for job in group:
                os.chdir(job.ws)
                # print(job)
                ...
                # print(density_list)
            filtered_temp_list = list(filter(None, temp_list))
            output_string = (
                "The average temp is {} K with SEM {} from {} samples".format(
                    np.mean(filtered_temp_list),
                    np.std(filtered_temp_list)
                    / np.sqrt(len(filtered_temp_list)),
                    len(filtered_temp_list),
                )
            )
            print(output_string)
            os.chdir(base_dir)
            with open("temp_results.txt", "w") as text_file:
