...
def hbond(job):
    ...
    with job:
        if job.sp.molecule == "ethanolAA":
            ...
            nbins_r = 200 ; nbins_a = 200 ; r_cutoff = 0.75 ; skip_every_x_frames = 10
            sel_oxygen_head = 'name O' ; sel_hydrogen = 'name H' ; sel_oxygen_tail = 'name O'; list_names_hydrogen = ["H"] ; list_names_oxygen_head = ["O"] ; list_names_oxygen_tail = ["O"]
            rdf_output, inter_output, map_output,hbond,hbond_time = calualateHBMap(traj, r_cutoff, nbins_r, nbins_a, skip_every_x_frames, sel_oxygen_head, sel_oxygen_tail, sel_hydrogen, list_names_hydrogen, list_names_oxygen_head, list_names_oxygen_tail, bonded_pdb_provided = True)
            inter_output[0] = inter_output[0]*180/np.pi
            plt.figure() ; cmap = plt.get_cmap('jet') ; plt.figure(figsize=(5, 3)) ; plt.style.use('default'); levels = np.linspace(0,100,11) ; cs = plt.contourf(rdf_output[0], inter_output[0], map_output,levels = levels, cmap=cmap) ; plt.xlabel('r (nm)') ; plt.ylabel('\u03B8 (degrees)') ; plt.xlim([0.2, 0.4]) ; plt.ylim([140, 180]) ; plt.colorbar()

            plt.savefig("hbond.png")
            np.savetxt(os.path.join(job.ws,'map_output.csv'),map_output,delimiter=",")
