...
def main():
    ...
    for (
        ...
        if "NPT" in ensemble and molecule == "ethanolAA":
            ...
            plt.figure(figsize=(7, 5)) ;  levels = np.linspace(1,51,11) ; 

            cs = plt.contourf(avg_r, avg_theta, avg_map_output,levels = levels, cmap=cmap) ; plt.xlabel(r'$r$'+r" ($\mathrm{\AA}$)", fontsize = xlabelfs) ; plt.ylabel('\u03B8 (degrees)', fontsize = ylabelfs) ; plt.xlim([2.2, 3.5]) ; plt.ylim([125, 180]) 
        ...
        (2.75, 180), 0.5*2, 50*2, ec="magenta", facecolor="none", linewidth=3
    )
            plt.gca().add_patch(ellipse)
            plt.grid(alpha = 0.25)

            plt.tight_layout()
            plt.text(3.2, 130, r"$n_\mathrm{HB}=$"+"{:.1f}".format(avg_n_hbond)+r"$\pm$"+"{:.1f}".format(sem), horizontalalignment='center', verticalalignment='center', fontsize = 14)
