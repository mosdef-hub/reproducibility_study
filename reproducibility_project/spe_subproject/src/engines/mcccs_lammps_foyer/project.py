...
@Project.label
@Project.pre(lambda j: j.sp.engine == "mcccs_lammps_foyer")
def files_ready(job):
...
def copy_files(job):
    ...
    for file in glob(
        ...
        + "/src/engine_input/mcccs_lammps_foyer/{}/{}/fort.4.*".format(
...
def copy_topmon(job):
    """Copy topmon.inp from root directory to mcccs_lammps_foyer directory."""
    print("Copying topmon.")
    shutil.copy(
        Project().root_directory()
        + "/src/engine_input/mcccs_lammps_foyer/{}/{}/topmon.inp".format(
            job.sp.molecule, job.sp.ensemble
        ),
        job.workspace() + "/",
    )
...
@Project.operation
@Project.pre(
    lambda j: j.sp.engine == "mcccs_lammps_foyer" and j.sp.ensemble == "NPT"
)
...
@Project.operation
@Project.pre(
    lambda j: j.sp.engine == "mcccs_lammps_foyer"
    and j.sp.ensemble == "GEMC-NVT"
)
@Project.pre(has_fort_files)
@Project.post(files_ready)
...
def make_restart_file(job):
    ...
    box_of_molecules = mb.load(
        Project().root_directory()
        + "/src/system_snapshots/{}.json".format(job.sp.molecule)
    )
    system = construct_system(job.sp, constrain=True)
    system[0].xyz = box_of_molecules.xyz
